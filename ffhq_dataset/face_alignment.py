...
def image_align(src_file, dst_file, face_landmarks, output_size=1024, transform_size=4096, enable_padding=True, x_scale=1, y_scale=1, em_scale=0.1, alpha=False):
    ...
    if shrink > 1:
        ...
        img = img.resize(rsize, PIL.Image.LANCZOS)  # Updated from ANTIALIAS to LANCZOS
        quad /= shrink
        qsize /= shrink
    ...
    if output_size < transform_size:
        img = img.resize((output_size, output_size), PIL.Image.LANCZOS)  # Updated from ANTIALIAS to LANCZOS
